...
export default function SitePreview({ site }: { site: GeneratedSite }) {
  ...
  return (
    <div className="flex flex-col gap-4 animate-fade-in-up">
      {/* Toolbar */}
      <div className="bg-frasier-card border border-frasier-border rounded-xl p-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="w-2.5 h-2.5 rounded-full bg-frasier-green inline-block" />
        ...
        {!isFallback && currentPage && (
          <a
            ...
            className="text-frasier-green text-sm hover:underline"
          >
            Open in new tab ↗
          </a>
        )}
          ...
          {site.pages.map((page, index) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium border transition-colors ${
                index === activePage
                  ? "bg-frasier-green text-black border-frasier-green"
                  : "bg-frasier-card border-frasier-border text-white/70 hover:text-white"
              }`}
            >
              {page.name}
            </button>
      ...
      {/* Iframe / Fallback */}
      {isFallback ? (
        <div className="bg-frasier-card border border-frasier-border rounded-xl flex items-center justify-center h-[600px]">
        ...
        <iframe
          ...
          className="w-full rounded-xl border border-frasier-border bg-white"
          style={{ height: "600px" }}
        />
      )}

      {/* Metadata */}
      <div className="bg-frasier-card border border-frasier-border rounded-xl p-4 flex flex-wrap gap-6">
